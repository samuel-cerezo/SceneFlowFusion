...
function [delta_V] = deltaV_ij(i,j,acel_lin,bias_ace,eta_ace,vel_ang,bias_gyr,eta_gyr,delta_t)  %Funcion corroborada OK
    ...
    for k=1:(j-i)
        delta_V = delta_V + deltaR_ij(1,k,vel_ang,bias_gyr,eta_gyr,delta_t)*(acel_lin(:,k) - bias_ace(:,k) - eta_ace(:,k))*delta_t;
    end
end
