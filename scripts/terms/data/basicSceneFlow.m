...
classdef basicSceneFlow < basicDualizedDataterm
    properties
        thisImage1;
        thisImage2;
    ...
    methods
        function obj = basicSceneFlow(alpha,image1,image2,varargin)
...
%            du = basicSceneFlow.generateDatatermParts(discretization,image1,image2,vTilde,termType,constancyDimension);
            du = basicSceneFlow.generateDatatermParts(discretization,image1,image2,vTilde,termType,constancyDimension);
            
            %   du{1} contiene dZ1/dx en forma de matriz
            %   du{2} contiene dZ1/dy en forma de matriz
            %   du{3} contiene Z1-Z0  en forma de matriz
...
%------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------            
            ...
            du_ant = du{end};
            du{end} = ones([size(image1),1]);
%            du{end} = zeros([size(image1),1]);
            du{end+1} = du_ant;       
            ...
            %   A{1,1} contiene dZ1/dx en forma de vector columna
            %   A{1,2} contiene dZ1/dy en forma de vector columna
            %   A{1,3} contiene 1's en forma de vector columna
    ...
    methods(Static)
        function du = generateDatatermParts(discretization,image1,image2,vTilde,termType,constancyDimension)
            ...
            [I2w,dI2w,ddI2w,dI1,markerOutOfGrid] = basicSceneFlow.generateGradients(discretization,image1,image2,vTilde);
            % Esta función computa las derivadas de la imagen que se le
            % ingresa.
            ...
            if (strcmp(termType,'brightnessConstancy'))
                ...
                du{end+1} = I2w - image1;   %Computa la resta de (imagen2-imagen1). El resultado lo guarda en la ultima celda de "du"
%                du{end+1} = I2w + image1;   %Computa la resta de (imagen2-imagen1). El resultado lo guarda en la ultima celda de "du"
