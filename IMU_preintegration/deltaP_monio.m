...
function [delta_P] = deltaP_monio(i,j,acel_lin,bias_ace,vel_ang,bias_gyr,delta_t)  %Funcion corroborada OK
    ...
    for k=1:(j-i)
        delta_P = delta_P + deltaV_monio(i,j,acel_lin,bias_ace,vel_ang,bias_gyr,delta_t)*delta_t  + 0.5*deltaR_monio(1,k,vel_ang,bias_gyr,delta_t)*(acel_lin(:,k) - bias_ace(:,k))*(delta_t^2);
    end
end
